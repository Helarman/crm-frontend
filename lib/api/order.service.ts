...
export const OrderService = {
   ...
   */
  create: async (dto: CreateOrderDto): Promise<OrderResponse> => {
  ...
  /**
   * Добавить модификатор заказа (дополнительную услугу)
   */
  addOrderAdditiveToOrder: async (
    orderId: string,
    orderAdditiveId: string,
    quantity: number = 1
  ): Promise<OrderResponse> => {
    try {
      ...
      console.error('Failed to add order additive to order:', error);
      throw error;
    }
  },

  /**
   * Удалить модификатор заказа из заказа
   */
  removeOrderAdditiveFromOrder: async (
    orderId: string,
    orderAdditiveId: string
  ): Promise<OrderResponse> => {
    try {
      ...
      console.error('Failed to remove order additive from order:', error);
      throw error;
    }
  },

  /**
   * Обновить модификаторы заказа (полная замена списка)
   */
